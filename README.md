...
**[Intent-Based Agent Governance — V1.0](./research/intent-based-agent-governance.md)** — Position paper arguing that AI agents need commander's intent, not just rules. Maps the governance deficits documented in "Agents of Chaos" (Shapira et al., 2026) to the intent-based framework independently developed and deployed in the Agent Harness Architecture three months before the study was conducted. The paper names the gap between technical enforcement and operational governance, and proposes a dual-layer architecture combining architectural hard limits with documented reasoning: mission context, stakeholder models, priority hierarchies, and the rationale behind every constraint. The harness projects in this portfolio are the proof of concept.

**[Intent-Based Agent Governance — V2.0](./research/intent-based-agent-governance-v2.md)** — Revised working paper incorporating post-publication findings. Key additions: (1) a new failure modes subsection addressing four pressure points — conflicting intents in multi-stakeholder environments, poor intent authorship, mis-specification bias, and prompt injection targeting the intent layer — framed as scope conditions, not weaknesses; (2) a new NIST AI RMF alignment section (Section 6), dated April 24, 2026, finding that the framework independently satisfies multiple NIST AI 100-1 and AI 600-1 sub-functions that NIST defines but does not specify how to implement — a third independent convergence data point; (3) the provenance record reframed to lead with independent convergence as the primary argument, with the forensic timestamp record moved to Appendix A; and (4) Recommendation 4 repositioned to frame full empirical validation as the research community's next contribution, requiring institutional resources beyond the scope of a working paper. Changes documented in the accompanying changelog.

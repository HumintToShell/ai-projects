...
Unlike RLHF-based mechanisms such as calibrated abstention and refusal training — which shape agent behavior at the training layer — this framework's contribution operates at runtime. The governance distinction matters: permission to fail, explicitly documented in operating context, redefines what the agent considers a successful outcome during inference rather than instilling that behavior through training. Traditional software fails deterministically: when a process encounters an unhandled error state, execution halts. A large language model encountering an analogous situation does not halt — it generates a response. Without explicit governance specifying otherwise, that response attempts to satisfy the task through improvisation. This is the mechanism behind the failures documented in the empirical literature. Large language models exhibit a strong bias toward producing a result even when the correct action is to produce no result at all. Under specified conditions, stopping the task, reporting that the situation exceeds the agent's authority, and preserving the current state *is* the successful outcome. Without this explicit redefinition in the governance layer, the agent's own optimization pressure pushes it toward improvisation even when the intent framework would otherwise support escalation or retreat. Architectural guardrails constrain what the agent can do. Permission to fail, documented as intent, constrains what the agent feels compelled to attempt.
...
A working implementation of this framework exists, independently developed and deployed in November 2025 under real-world operational constraints — not designed against a research specification or benchmark (see Appendix A). This implementation reflects a single-operator environment under specific constraints and has not been evaluated through controlled comparative testing. Its role here is to demonstrate feasibility and operational coherence, not to establish statistical performance claims. The design decisions that address the governance deficits documented in the empirical literature emerged from applying operational doctrine to an operational problem. The convergence with subsequently published research is what establishes the framework's generality; the provenance is documented there.
...
A note on terminology before proceeding. This framework is not equivalent to ad hoc prompt engineering. Prompt engineering optimizes task performance through instruction phrasing. Intent-based governance introduces persistent, structured operational context that defines mission, authority, and decision criteria independent of any single task. The distinction is reproducibility: governance artifacts can be standardized, audited, and consistently applied across deployments, whereas prompts are typically ephemeral and task-specific.
...
The vocabulary already exists. The doctrine already exists. The failure modes are documented. A working proof of concept is deployed. What remains is empirical validation at scale — controlled comparison of agent behavior under rules-only versus rules-plus-intent governance — and systematic implementation across operational deployments. That is the research community's contribution. This paper offers the framework.

---

## References

Microsoft Security. (2026). Governing AI Agent Behavior: Aligning User, Developer, Role, and Organizational Intent. Microsoft Tech Community. https://techcommunity.microsoft.com/blog/microsoft-security-blog/governing-ai-agent-behavior-aligning-user-developer-role-and-organizational-inte/4503551

Microsoft Open Source. (2026, April 2). Introducing the Agent Governance Toolkit: Open-source Runtime Security for AI Agents. Microsoft Open Source Blog. https://opensource.microsoft.com/blog/2026/04/02/introducing-the-agent-governance-toolkit-open-source-runtime-security-for-ai-agents/

MYGOM. (2026). Why AI Governance in 2026 Needs More Than Rules. MYGOM.tech. https://mygom.tech/articles/ai-governance-in-2026-frameworks-are-not-enough-anymore
...
World Economic Forum. (2026, March). From Chatbots to Assistants: Governance Is Key for AI Agents. WEF Stories. https://www.weforum.org/stories/2026/03/ai-agent-autonomy-governance/

---

## Appendix A: Provenance Record

The thesis of this appendix is not chronological priority. It is independent development. A framework that emerges from operational practice and then converges with empirical research and federal standards it was not designed against carries evidential weight that a derivative framework does not. The provenance record below establishes the conditions of that independence.

The agentic harness described in this paper was independently designed and deployed in November 2025. The earliest architectural documents carry modification timestamps from November 1, 2025 — twenty-three days before OpenClaw's first GitHub commit on November 24, 2025, and before the research documented in "Agents of Chaos," published February 2026. The primary governance implementation file carries a modification timestamp of November 23, 2025, one day before that initial external parallel. Internal document content referencing November 2025 dates corroborates the metadata.

The relevance of this record is independence, not priority. The framework was developed without knowledge of the "Agents of Chaos" study, the NIST AI RMF sub-functions it was later found to satisfy, or any comparable published governance approach. The subsequent convergence with multiple independent bodies of work — empirical research, federal standards, and open-source development — is what the provenance establishes: parallel solutions to the same problem, not derivative work.

The timeline is established through filesystem metadata preserved in the original development environment. The governance documents were authored within a Linux virtual machine whose disk image has been forensically acquired via ewfacquire (libewf). For readers outside the forensic discipline: ewfacquire is a sector-by-sector imaging tool that produces an E01 (Expert Witness Format) file — a tamper-evident copy of the disk that preserves all filesystem metadata including ext4 timestamps for file modification, access, and change times. These timestamps cannot be altered post-acquisition without detection. This forensic record is available for independent verification.
